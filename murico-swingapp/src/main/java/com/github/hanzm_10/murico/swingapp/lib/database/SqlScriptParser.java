...
public final class SqlScriptParser {
	public static record SqlStatement(@NotNull String type, @NotNull String name, @NotNull String sql) {
	}
	...
	public static SqlStatement parseSqlStatement(String script) {
		...
		if (matcher.find()) {
			...
			String type = (objectType != null) ? (action + " " + objectType.toUpperCase()) // e.g. CREATE TRIGGER
					: action;
