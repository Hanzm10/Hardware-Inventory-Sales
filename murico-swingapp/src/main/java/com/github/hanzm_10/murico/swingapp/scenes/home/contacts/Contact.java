...
public class Contact {
	...
	public void displayUsersTableForNotAdmin(DefaultTableModel tableModel, Set<Integer> editedRows) {
		...
		try {
			...
			for (var user : users) {
				Object[] rowData = { user._userId(), user.displayName(), user.roles(), user.verificationStatus() };
				tableModel.addRow(rowData);
			}
		} catch (IOException | SQLException e) {
			// TODO Auto-generated catch block
	...
	public void saveChanges(DefaultTableModel model, Set<Integer> editedRows) {
		...
		try {
			...
			var updateQuery = MySqlQueryLoader.getInstance().get("update_userScenes_table", "accounts",
					MySqlQueryLoader.SqlQueryType.UPDATE);
