...
public class RestockItemDialog extends JDialog {
	...
	public RestockItemDialog(Window owner, InventorySceneNew parent, int itemStockId, int itemId, String productName,
		...
		// Main Content Panel
		// IMPORTANT: createInputPanel() initializes supplierLabelValue,
		// unitPriceLabelValue, etc.
		...
		// Listener for quantity spinner (needs wholesalePrice to be set by
		// fetchSupplierAndWsp)
	...
	private void confirmOrderAction() {
		...
		if (quantityToOrder <= 0) {
			JOptionPane.showMessageDialog(this, "Please enter a quantity greater than zero to order.",
	...
	private void fetchSupplierAndWsp() {
		...
		try (Connection conn = MySqlFactoryDao.createConnection();
			...
			wholesalePrice = BigDecimal.ZERO;
			unitPriceLabelValue.setText(CURRENCY_FORMAT.format(wholesalePrice));
			JOptionPane.showMessageDialog(this, "Error fetching supplier details: " + e.getMessage(), "Database Error",
	...
	private void updateOrderDisplay() {
		...
		totalOrderPriceLabelValue.setText(CURRENCY_FORMAT.format(lineTotal)); // Assuming only one item type per restock
																				// dialog
	}
}