...
public class TransactionHistoryPanel extends JPanel {
	...
	private void initializeUI() {
		...
		transactionHistoryTableModel = new DefaultTableModel(cols, 0) {
			...
			public Class<?> getColumnClass(int columnIndex) {
				...
				case TRANS_COL_AMOUNT:
					return BigDecimal.class;
				case TRANS_COL_REF_ID:
	...
	/**
	 * Fetches and displays combined transaction history. NOTE: This query is
	 * complex and assumes certain interpretations. Excludes item_restocks and
	 * refunds for now based on schema complexity.
	 */
		...
		try (Connection conn = MySqlFactoryDao.createConnection(); // Use correct factory
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					...
					row.add(rs.getObject("ref_id")); // Use getObject for potentially different types (INT vs UUID maybe
														// later)
