...
public class AddItemDialog extends JDialog {
    ...
    public AddItemDialog(Window owner, InventoryScene parent) {
        ...
        JButton confirmButton = createStyledButton("Save Item", new Color(0x00695C), Color.WHITE); // Tealish (was "Confirm Product")
    ...
    // --- Helper methods for creating styled components (mostly from previous version) ---
    ...
    private JComboBox<ComboBoxItem> createStyledComboBox() {
        ...
        // For truly custom arrow/border like mockup, L&F customization or custom UI delegate needed
    ...
    private void saveNewItem() {
        ...
        if (sellingPrice.compareTo(BigDecimal.ZERO) <= 0) {
            JOptionPane.showMessageDialog(this, "Selling Price must be greater than zero.", "Validation Error", JOptionPane.WARNING_MESSAGE);
            sellingPriceSpinner.requestFocusInWindow(); return;
        }
        if (srp.compareTo(BigDecimal.ZERO) < 0) { // SRP can be 0
            JOptionPane.showMessageDialog(this, "SRP cannot be negative.", "Validation Error", JOptionPane.WARNING_MESSAGE);
            srpSpinner.requestFocusInWindow(); return;
        }
        if (initialQty < 0 || minQty < 0) {
            ...
            (initialQty < 0 ? initialQuantitySpinner : minQuantitySpinner).requestFocusInWindow(); return;
        }
        if (selectedSupplier != null && selectedSupplier.getId() != -1 && costPrice.compareTo(BigDecimal.ZERO) < 0) {
             JOptionPane.showMessageDialog(this, "If a supplier is selected, Cost Price cannot be negative.", "Validation Error", JOptionPane.WARNING_MESSAGE);
             costPriceSpinner.requestFocusInWindow(); return;
        }
        if ((selectedSupplier == null || selectedSupplier.getId() == -1) && costPrice.compareTo(BigDecimal.ZERO) > 0) {
            JOptionPane.showMessageDialog(this, "Please select a supplier if providing a Cost Price.", "Validation Error", JOptionPane.WARNING_MESSAGE);
        ...
        try {
            ...
            // Assuming 'items' table DOES NOT have a 'brand' column based on original schema.
            ...
            e.printStackTrace();
            if (conn != null) { try { conn.rollback(); } catch (SQLException excep) { excep.printStackTrace(); } }
    ...
    private static class ComboBoxItem {
        ...
        public ComboBoxItem(int id, String name) { this.id = id; this.name = name; }
