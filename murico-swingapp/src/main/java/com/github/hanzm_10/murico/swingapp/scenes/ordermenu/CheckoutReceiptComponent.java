...
public class CheckoutReceiptComponent extends JPanel {
	...
	// String Format Widths (adjust based on desired columns with
	// Monospaced/Courier)
	...
	/**
	 * Calculates the approximate required height for the PDF receipt based on
	 * content.
	 *
	...
	private void initializeReceiptUI() {
		...
		printReceiptButton.addActionListener(e -> {
			...
			if (parent instanceof CheckoutPanel checkoutParent) {
				...
				JOptionPane.showMessageDialog(this, "Error: Cannot access checkout data to print.", "Internal Error",
	...
	/**
	 * Public method to trigger PDF generation. Called externally by CheckoutPanel
	 * or internally by button action listener.
	 *
	...
	/**
	 * Internal method to handle PDF generation and saving using Apache PDFBox. Uses
	 * Courier font for alignment. Uses PDF_CURRENCY_FORMAT (no symbol).
	 *
	...
	private void printReceiptInternal(DefaultTableModel tableModel, int orderId) {
		...
		// --- Suggest Filename ---
		String defaultFileName = (orderId > 0) ? "Receipt-" + orderId + ".pdf"
		...
		if (userSelection == JFileChooser.APPROVE_OPTION) {
			...
			try (PDDocument document = new PDDocument()) {
				...
				java.util.function.BiFunction<String, Float, Float> getTextWidth = (text, fontSize) -> {
					...
					} // Use fontRegular (Courier)
					catch (IOException e) {
						System.err.println("Err width: " + e);
						return 10f;
					}
				};
				java.util.function.BiFunction<String, Float, Float> getBoldTextWidth = (text, fontSize) -> {
					try {
						return fontBold.getStringWidth(text) / 1000 * fontSize;
					} // Use fontBold (Courier Bold)
					catch (IOException e) {
						System.err.println("Err bold width: " + e);
						return 10f;
					}
				};
				...
				try {
					...
					String line1 = "Murico Branch #" + currentBranchId;
					float line1X = margin + (contentWidth - getBoldTextWidth.apply(line1, FONT_SIZE_HEADER)) / 2;
					contentStream.setFont(fontBold, FONT_SIZE_HEADER);
					contentStream.newLineAtOffset(line1X, yPosition);
					contentStream.showText(line1);
					yPosition -= LINE_SPACING;
					currentX = line1X;
					contentStream.setFont(fontRegular, FONT_SIZE_NORMAL);
					String receiptNumStr = (orderId > 0) ? String.valueOf(orderId) : "[Preview]";
					String line2 = "Receipt #: " + receiptNumStr;
					float line2X = margin + (contentWidth - getTextWidth.apply(line2, FONT_SIZE_NORMAL)) / 2;
					contentStream.newLineAtOffset(line2X - currentX, -LINE_SPACING);
					contentStream.showText(line2);
					yPosition -= LINE_SPACING;
					currentX = line2X;
					String line3 = "Clerk: [Employee Name Placeholder]";
					float line3X = margin + (contentWidth - getTextWidth.apply(line3, FONT_SIZE_NORMAL)) / 2;
					contentStream.newLineAtOffset(line3X - currentX, -LINE_SPACING);
					contentStream.showText(line3);
					yPosition -= LINE_SPACING;
					currentX = line3X;
					String line4 = "Date: " + RECEIPT_DATE_FORMAT.format(new Date());
					float line4X = margin + (contentWidth - getTextWidth.apply(line4, FONT_SIZE_NORMAL)) / 2;
					contentStream.newLineAtOffset(line4X - currentX, -LINE_SPACING);
					contentStream.showText(line4);
					yPosition -= LINE_SPACING;
					currentX = line4X;
					contentStream.newLineAtOffset(margin - currentX, -LINE_SPACING * 0.5f);
					yPosition -= LINE_SPACING * 0.5f;
					currentX = margin; // Reset X
					...
					yPosition -= ITEM_LINE_SPACING;
					contentStream.newLineAtOffset(0, -ITEM_LINE_SPACING);
					String headerLine = String.format("%-" + wItem + "s%" + wQty + "s%" + wPrice + "s%" + wAmount + "s",
					...
					for (int i = 0; i < tableModel.getRowCount(); i++) {
						...
						if (yPosition < BOTTOM_MARGIN_PT + ITEM_LINE_SPACING) {
							/* ... handle page break ... */ }
						...
						if (name != null && quantity != null && itemPrice != null && quantity > 0) {
							...
							String itemLine = String.format(
									"%-" + wItem + "s%" + wQty + "d%" + wPrice + ".2f%" + wAmount + ".2f", displayName,
									quantity, itemPrice, amount);
					...
					String totalLine = String.format("%-" + totalLabelWidth + "s%" + valueWidth + "s", totalText,
							totalValueStr);
					...
					yPosition -= LINE_SPACING;
					contentStream.newLineAtOffset(0, -LINE_SPACING); // Extra space
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(parentWindow, "PDF IO Error: " + e.getMessage(), "Error", 0);
			} catch (Exception e) {
				e.printStackTrace();
				JOptionPane.showMessageDialog(parentWindow, "Unexpected PDF Error: " + e.getMessage(), "Error", 0);
			}
		} else {
	...
	/**
	 * Updates the receipt preview text area based on the provided table model. Uses
	 * String.format optimized for monospaced fonts and DISPLAY_CURRENCY_FORMAT.
	 *
	 * @param tableModel The current DefaultTableModel from the checkout table. Can
	 *                   be null.
	...
	public void updateReceipt(DefaultTableModel tableModel, int orderId) {
		...
		String headerLine = String.format(
				"%-" + WIDTH_ITEM + "s%" + WIDTH_QTY + "s%" + WIDTH_PRICE + "s%" + WIDTH_AMOUNT + "s", "Item", "Qty",
				"Price", "Amount");
			...
			for (int i = 0; i < tableModel.getRowCount(); i++) {
				try {
					...
					if (name != null && quantity != null && price != null && quantity > 0) {
						...
						String itemLine = String.format(
								"%-" + WIDTH_ITEM + "s%" + WIDTH_QTY + "d%" + WIDTH_PRICE + "s%" + WIDTH_AMOUNT + "s",
								displayName, quantity, DISPLAY_CURRENCY_FORMAT.format(price), // Format price with
																								// symbol
