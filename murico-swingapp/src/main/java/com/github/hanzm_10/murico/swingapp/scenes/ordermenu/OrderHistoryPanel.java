...
public class OrderHistoryPanel extends JPanel {
    ...
    private void initializeUI() {
        ...
        orderHistoryTableModel = new DefaultTableModel(cols, 0) {
            ...
            @Override public Class<?> getColumnClass(int columnIndex) {
                 switch (columnIndex) {
                     case HISTORY_COL_DATE: return Timestamp.class;
                     case HISTORY_COL_ORDER_ID: return Integer.class;
                     case HISTORY_COL_TOTAL: return String.class; // Display formatted currency
                     // case HISTORY_COL_TOTAL: return BigDecimal.class; // If sorting by raw value needed
    ...
    private void loadOrderHistoryData() {
        if (orderHistoryTableModel == null) return; // Check if UI is ready
        ...
        String sql = """
            SELECT
                ...
                s.total_price_php        -- Get total cost from SALES table
        ...
        try (Connection conn = MySqlFactoryDao.createConnection(); // Use correct factory
             ...
             try(ResultSet rs = pstmt.executeQuery()) {
                 while(rs.next()) {
                     ...
                     BigDecimal total = rs.getBigDecimal("total_price_php");
