...
// --- Imports ---
import com.github.hanzm_10.murico.swingapp.scenes.ordermenu.CheckoutPanel; // Import new panels
import java.awt.event.MouseAdapter;
import java.awt.event.MouseEvent;
import java.awt.event.MouseListener;
import com.github.hanzm_10.murico.swingapp.scenes.ordermenu.OrderHistoryPanel;
import com.github.hanzm_10.murico.swingapp.scenes.ordermenu.TransactionHistoryPanel;
import java.awt.*;
import javax.swing.*;
...
public class OrderMenuScene extends JPanel implements Scene {
    ...
    @Override public String getSceneName() { return "ordermenu"; }
    ...
    public void onCreate() {
        ...
        if (!uiInitialized) {
            ...
            checkoutPanel = new CheckoutPanel(currentBranchId); // Keep branchId HERE if CheckoutPanel still needs it for display/logging
    ...
    public void onShow() {
        ...
        // checkoutPanel.loadData(); // Add loadData method to CheckoutPanel if needed for reset
    }

    @Override public void onHide() { /* ... */ }
    @Override public boolean onDestroy() { removeAll(); uiInitialized = false; /* Call destroy on sub-panels? */ return true; }
    ...
    private void initializeOrderMenuUI() {
        ...
        // ... (Top bar with title and notification icon) ...
         JPanel topBar = new JPanel(new BorderLayout()); topBar.setBorder(new EmptyBorder(5, 10, 5, 10)); JLabel titleLabel = new JLabel("ORDER MENU"); titleLabel.setFont(new Font("Montserrat ExtraBold", Font.BOLD, 24)); titleLabel.setHorizontalAlignment(SwingConstants.CENTER); JLabel notificationLabel = new JLabel("\uD83D\uDD14"); notificationLabel.setFont(new Font("Segoe UI Symbol", Font.PLAIN, 20)); topBar.add(titleLabel, BorderLayout.CENTER); topBar.add(notificationLabel, BorderLayout.EAST); topSectionPanel.add(topBar, BorderLayout.NORTH);
        ...
        styleNavButton(checkoutButton); addHoverEffect(checkoutButton);
        checkoutButton.addActionListener(e -> showCard(CARD_CHECKOUT)); // Use helper

        styleNavButton(orderHistoryButton); addHoverEffect(orderHistoryButton);
        orderHistoryButton.addActionListener(e -> showCard(CARD_ORDER_HISTORY)); // Use helper

        styleNavButton(transactionHistoryButton); addHoverEffect(transactionHistoryButton);
    ...
    private void showCard(String cardName) {
        ...
        switch (cardName) {
            case CARD_ORDER_HISTORY:
                if (orderHistoryPanel != null) orderHistoryPanel.loadData();
                break;
            case CARD_TRANSACTION_HISTORY:
                if (transactionHistoryPanel != null) transactionHistoryPanel.loadData();
                break;
            case CARD_CHECKOUT:
    ...
    private void styleNavButton(JButton button) { /* ... */ button.setFont(new Font("Montserrat Bold", Font.BOLD, 12)); button.setFocusPainted(false); button.setBorder(BorderFactory.createEmptyBorder(8, 15, 8, 15)); button.setOpaque(true); button.setContentAreaFilled(true); button.setBackground(UIManager.getColor("Button.background")); button.setForeground(UIManager.getColor("Button.foreground")); button.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); }
    private void addHoverEffect(JButton button) { /* ... */ Color originalBg = button.getBackground(); button.addMouseListener(new MouseAdapter() { @Override public void mouseEntered(MouseEvent e) { button.setBackground(NAV_BUTTON_HOVER_COLOR); } @Override public void mouseExited(MouseEvent e) { button.setBackground(originalBg); } }); }
