...
public class CheckoutService {
 ...
 public int finalizeOrder(List<OrderLineItemData> items,
     ...
     try {
         ...
         if (conn == null) throw new SQLException("Failed database connection.");
         ...
         try (PreparedStatement pstmtOrder = conn.prepareStatement(sqlOrder, Statement.RETURN_GENERATED_KEYS)) {
             ...
             if (rowsAffected == 0) throw new SQLException("Creating customer order failed, no rows affected.");
             ...
             try (ResultSet generatedKeys = pstmtOrder.getGeneratedKeys()) {
                 ...
                 } else { throw new SQLException("Creating customer order failed, no ID obtained."); }
             }
         } // Closes pstmtOrder
         ...
         try (PreparedStatement pstmtItemInsert = conn.prepareStatement(sqlItemInsert);
             ...
             for (OrderLineItemData item : items) {
                 if (item.quantity() <= 0) { throw new SQLException(/*...*/); }
             ...
             int[] stockUpdateCounts = pstmtStockUpdate.executeBatch(); /* Validate, throw InsufficientStockException if needed */
              for(int c : itemInsertCounts) { if(c == Statement.EXECUTE_FAILED) throw new SQLException("Item insert batch failed."); }
              if (stockUpdateCounts.length != items.size()) { throw new SQLException("Stock update batch size mismatch."); }
              for (int i = 0; i < stockUpdateCounts.length; i++) { if (stockUpdateCounts[i] == Statement.EXECUTE_FAILED) throw new SQLException("Stock update batch execution failed."); if (stockUpdateCounts[i] == 0) { OrderLineItemData failedItem = items.get(i); String errorMsg = "Insufficient stock for item '" + failedItem.itemName() + "' (StockID: " + failedItem.itemStockId() + "). Order cannot be completed."; LOGGER.log(Level.WARNING, errorMsg); throw new InsufficientStockException(errorMsg); } }
